...
public class RootbeerTestAgent {
  ...
  public void test(ClassLoader cls_loader, boolean run_hard_tests) throws Exception {
    ...
    List<TestKernelTemplate> kernel_template_creators = loader.loadKernelTemplate(cls_loader, "edu.syr.pcratts.rootbeer.test.KernelTemplateMain");
    int num_tests = creators.size() + ex_creators.size() + change_thread.size() + kernel_template_creators.size();
    int test_num = 1;

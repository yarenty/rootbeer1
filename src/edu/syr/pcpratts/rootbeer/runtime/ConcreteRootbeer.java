...
public class ConcreteRootbeer implements IRootbeerInternal {
	...
	private void runKernelTemplateJava(Kernel kernel) {
		...
		for (int i = 0; i < m_threadConfig.getBlockShapeX()
			...
			while (templateThreadListsProvider.getSleeping().isEmpty())
				;
			TemplateThread t = templateThreadListsProvider.getSleeping()
					.remove(0);
			t.kernel = kernel;
			t.m_blockIdxx = 0;
			t.startid = i;
	...
	public List<GpuCard> getGpuCards() {
		return CudaRuntime2.v().getGpuCards();
	}

	@Override
	public void setCurrentGpuCard(GpuCard gpuCard) {
		CudaRuntime2.v().setCurrentGpuCard(gpuCard);
	}

	@Override
	public GpuCard getCurrentGpuCard() {
		return CudaRuntime2.v().getCurrentGpuCard();
	}
}
